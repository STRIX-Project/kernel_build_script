 ...
 clone() {
	...
	then
		if [[ "$CI_BRANCH" == "sdm660-hmp-rebase" ]]; then
			...
			msg "|| Cloning GCC 11.0 eksperimental ||"
			git clone https://github.com/fiqri19102002/aarch64-gcc.git -b elf-gcc-master --depth=1 gcc64
			git clone https://github.com/fiqri19102002/arm-gcc.git -b elf-gcc-master --depth=1 gcc32
			GCC64_DIR=$KERNEL_DIR/gcc64
			GCC32_DIR=$KERNEL_DIR/gcc32
		fi
	fi
